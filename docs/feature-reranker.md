...
- brand evidence or model-token overlap of at least `0.25`;
- combined score of at least `0.40`;
- top-1 versus top-2 margin of at least `0.08`.

The weights and thresholds were fixed before the first semantic-dev run. Every
candidate's feature vector is retained in the JSON report.
...
| Feature reranker v0.1 | **0.900** | **1.000** | **0.920** | **1.000** | **0.720** |
...
Local CPU query latency was approximately p50 `21.6 ms` and p95 `23.1 ms`, dominated
by MiniLM inference. Model loading took approximately `6.4 s`.
